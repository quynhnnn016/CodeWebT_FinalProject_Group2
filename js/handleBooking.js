...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function autoFillUserInfo() {
    ...
    if (currentUser && bookingForm) {
      const fullNameInput = bookingForm.querySelector(
        'input[placeholder="Full Name *"]'
      );
      const phoneInput = bookingForm.querySelector(
        'input[placeholder="Phone Number *"]'
      );
      const emailInput = bookingForm.querySelector(
        'input[placeholder="Email *"]'
      );
      const addressInput = bookingForm.querySelector(
  ...
  // --- Get Tour Details from right summary box ---
  function getTourDetails() {
    const summaryBox = document.querySelector(".summary-box");
    if (!summaryBox) return null;

    const tourNameElement = summaryBox.querySelector("h5.fw-bold");
    const tourLocationElement = summaryBox.querySelector(
      "p.text-muted i.fa-map-marker-alt"
    );
    const tourPriceElement = summaryBox.querySelector(
      ".total-box .text-danger"
    );
    const tourImageElement = summaryBox.querySelector("img");

    return {
      name: tourNameElement
        ? tourNameElement.textContent.trim()
        : "Unknown Tour",
      location: tourLocationElement
        ? tourLocationElement.parentElement.textContent
            .trim()
            .replace("Location:", "")
        : "Unknown Location",
  ...
  if (confirmButton && bookingForm) {
    confirmButton.addEventListener("click", (event) => {
      ...
      const passengerDivs = bookingForm.querySelectorAll(
      ...
      passengerDivs.forEach((div) => {
        ...
        if (label && input) {
          ...
          if (labelText.includes("adults")) adultsInput = input;
          else if (labelText.includes("children")) childrenInput = input;
          else if (labelText.includes("infants")) infantsInput = input;
        }
      });
      ...
      // 5️⃣ Get selected departure date (from localStorage)
      const selectedDeparture = JSON.parse(
        localStorage.getItem("TriplySelectedDeparture")
      );
      if (!selectedDeparture) {
        alert("Please select a departure date before confirming your booking.");
        return;
      }

      // 6️⃣ Create booking object
      const newBooking = {
        bookingId:
          "TRIPLY-" + Date.now() + Math.random().toString(16).substring(2, 8),
        ...
        departureDate: `${selectedDeparture.day}/${selectedDeparture.month}`,
      ...
      try {
        ...
        // Clear selected departure
        localStorage.removeItem("TriplySelectedDeparture");
